...
{
	if ( IsPC() && m_bOpenProgressOnStart )
	{
		StartProgressBar();
	...
	if ( IsX360() && m_bPlayGameStartupSound )
	{
...
void CGameUI::OnLevelLoadingFinished(bool bError, const char *failureReason, const char *extendedReason)
{
	...
	//Stavaas - if we will not fix that standart background in loading dialog while level transition - disable that:
