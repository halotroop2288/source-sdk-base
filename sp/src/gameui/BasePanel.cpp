...
//1080p
#ifdef _X360UI1080p
	#define MAIN_MENU_INDENT_X360 15
#endif

//1440p
#ifdef _X360UI1440p
	#define MAIN_MENU_INDENT_X360 20
#endif

//2160p
#ifdef _X360UI2160p
	#define MAIN_MENU_INDENT_X360 30
#endif
...
//ELSE
	...
	{
		SetArmedColor(GetSchemeColor("MainMenu.ArmedTextColor", pScheme), GetSchemeColor("MainMenu.ArmedBgColor", pScheme));

		//STAVAAS Res
	
	//720p
#ifdef _X360UI720p
		SetTextInset( MAIN_MENU_INDENT_X360, -1 );
#else
...
void CGameMenuItem::PaintBackground()
{
	...
	//1080p
#ifdef _X360UI1080p
		DrawBoxFade(3, 3, wide - 6, tall - 6, Color(0, 0, 0, 96), 1.0f, 255, 0, true);
#endif

	//1440p
#ifdef _X360UI1440p
		DrawBoxFade(4, 4, wide - 8, tall - 8, Color(0, 0, 0, 96), 1.0f, 255, 0, true);
#endif

	//2160p
#ifdef _X360UI2160p
		DrawBoxFade(6, 6, wide - 12, tall - 12, Color(0, 0, 0, 96), 1.0f, 255, 0, true);
#endif
...
public:
	...
	virtual void OnKeyCodePressed( KeyCode code )
	{
		...
		if (code == KEY_XBUTTON_UP || code == KEY_XBUTTON_DOWN)
		{
			input()->SetCursorPos(wide, tall);

			OnKeyCodeTyped(code);
			return;
		}
		if (code == KEY_XBUTTON_A || code == KEY_ENTER)
		{
			input()->SetCursorPos(wide, tall);
...
void CBasePanel::RunFrame()
{
	...
	if ( GameUI().IsConsoleUI() )
	{
		...
		if ( IsPC() && m_ExitingFrameCount && engine->Time() >= m_flTransitionEndTime )
		{
	...
	if ( !m_bPlatformMenuInitialized )
	{
		...
		if ( IsPC() || g_VModuleLoader.IsPlatformReady() )
		{
...
void CBasePanel::ApplySchemeSettings(IScheme *pScheme)
{
	...
	// PC uses FullFrameFB RT !!!
	m_iRenderTargetImageID = surface()->CreateNewTextureID();
	surface()->DrawSetTextureFile( m_iRenderTargetImageID, "console/rt_background", true, false );
...
#ifdef _X360UI1080p
	m_nButtonGap = 48;
	m_nButtonGapDefault = 48;
	m_ButtonPinRight = 150;
	m_FooterTall = 120;
#endif
...
#ifdef _X360UI1440p
	m_nButtonGap = 64;
	m_nButtonGapDefault = 64;
	m_ButtonPinRight = 200;
	m_FooterTall = 160;
#endif
...
#ifdef _X360UI2160p
	m_nButtonGap = 96;
	m_nButtonGapDefault = 96;
	m_ButtonPinRight = 300;
	m_FooterTall = 240;
#endif
...
//480p
	...
	m_ButtonPinRight = inResourceData->GetInt( "button_pin_right", 80 );
	m_ButtonSeparator = inResourceData->GetInt( "button_separator", 4 );
	m_FooterTall = inResourceData->GetInt( "tall", 60 );
#endif
...
//1080p
#ifdef _X360UI1080p
	m_nButtonGap = inResourceData->GetInt("buttongap", 48);
	m_ButtonPinRight = inResourceData->GetInt("button_pin_right", 150);
	m_ButtonSeparator = inResourceData->GetInt("button_separator", 6);
	m_FooterTall = inResourceData->GetInt("tall", 120);
#endif

//1440p
#ifdef _X360UI1440p
	m_nButtonGap = inResourceData->GetInt("buttongap", 64);
	m_ButtonPinRight = inResourceData->GetInt("button_pin_right", 200);
	m_ButtonSeparator = inResourceData->GetInt("button_separator", 8);
	m_FooterTall = inResourceData->GetInt("tall", 160);
#endif

//2160p
#ifdef _X360UI2160p
	m_nButtonGap = inResourceData->GetInt("buttongap", 96);
	m_ButtonPinRight = inResourceData->GetInt("button_pin_right", 300);
	m_ButtonSeparator = inResourceData->GetInt("button_separator", 12);
	m_FooterTall = inResourceData->GetInt("tall", 240);
#endif
...
void CMessageDialogHandler::ShowMessageDialog( int nType, vgui::Panel *pOwner )
{
	...
	case MD_SAVING_WARNING:
		CreateMessageDialog(MD_WARNING | iSimpleFrame | MD_COMMANDAFTERCLOSE | MD_RESTRICTPAINT,
							 "",
	...
	case MD_SAVE_COMPLETE:
		CreateMessageDialog(MD_OK | iSimpleFrame | MD_COMMANDAFTERCLOSE | MD_RESTRICTPAINT,
							 "#GameUI_ConfirmOverwriteSaveGame_Title", 
							 "#GameUI_GameSaved", 
