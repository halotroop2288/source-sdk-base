...
void CLoadingDialog::PaintBackground()
{
	...
	{
		color = GetSchemeColor( "TanDarker", Color(255, 255, 255, 255), vgui::scheme()->GetIScheme(scheme) );
...
//1080p
#ifdef _X360UI1080p
	DrawBox( x+3, y+3, barWide-6, barTall-6, Color( 100, 100, 100, 255 ), 1.0f );
#endif

//1440p
#ifdef _X360UI1440p
	DrawBox( x+4, y+4, barWide-8, barTall-8, Color( 100, 100, 100, 255 ), 1.0f );
#endif

//2160p
#ifdef _X360UI2160p
	DrawBox( x+6, y+6, barWide-12, barTall-12, Color( 100, 100, 100, 255 ), 1.0f );
#endif


	barWide = m_flProgressFraction * ( barWide - 4 );

	color = GetSchemeColor("ProgressColor", Color(220, 220, 220, 255), vgui::scheme()->GetIScheme(scheme));

	// cannot draw a curved box smaller than XX pixels without artifacts
	...
	if ( barWide >= 16 )
	{
		DrawBox( x + 2, y + 2, barWide, barTall - 4, color, 1.0f );
	}
#endif
	...
	if ( barWide >= 30 )
	{
		DrawBox( x + 3, y + 3, barWide, barTall - 6, color, 1.0f );
	}
#endif
	...
	if ( barWide >= 32 )
	{
		DrawBox( x + 4, y + 4, barWide, barTall - 8, color, 1.0f );
	}
#endif
	...
	if ( barWide >= 64 )
	{
		DrawBox( x + 6, y + 6, barWide, barTall - 12, color, 1.0f );
	}
#endif
...
void CLoadingDialog::PerformLayout()
{
	...
	input()->SetCursorPos( screenWide, screenTall );
...
bool CLoadingDialog::SetProgressPoint( float fraction )
{
	if ( m_bConsoleStyle )
	{
		...
		if ( (int)(fraction * 200) != (int)(m_flProgressFraction * 200) )
		{
	...
	return (nOldDrawnSegments != nNewDrawSegments) || IsPC();
}
